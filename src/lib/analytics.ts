...
import { EnrollmentStatus, TestType } from "@/generated/client";
...
export async function getAdminAnalytics() {
  ...
  const postTestScores = testAttempts
    .filter(a => a.test.type === TestType.POST && a.score !== null)
    .map(a => a.score)
  ...
  return {
    metrics: {
      totalKaryawan: await db.user.count({ where: { roles: { has: "KARYAWAN" } } }),
