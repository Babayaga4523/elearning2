...
export async function validateCourse(courseId: string): Promise<IntegrityResult> {
  ...
  if (!postTest) {
    ...
    postTest.questions.forEach((q, qIdx) => {
      ...
      // Validate: Must have exactly ONE correct answer
      if (correctCount === 0) {
        errors.push(`Post-Test Pertanyaan #${qIdx + 1}: Belum ada jawaban benar yang dipilih`);
      } else if (correctCount > 1) {
        errors.push(`Post-Test Pertanyaan #${qIdx + 1}: Hanya boleh ada 1 jawaban benar (saat ini: ${correctCount} jawaban benar)`);
      }
      ...
      q.options.forEach((opt, optIdx) => {
        if (!opt.text || opt.text.trim().length === 0) {
          errors.push(`Post-Test Pertanyaan #${qIdx + 1}, Opsi #${optIdx + 1}: Teks opsi tidak boleh kosong`);
        }
      });
    });
  }
