...
export async function getPerformanceData() {
  ...
  const courseAnalysis = enrollments.map(en => {
    ...
    const bestPost = courseAttempts
      .filter(a => a.test.type === "POST" && a.score !== null)
      .sort((a, b) => (b.score ?? 0) - (a.score ?? 0))[0];

    // Calculate growth delta based on BEST scores
    let growth = null;
    if (bestPre && bestPost && bestPre.score !== null && bestPost.score !== null) {
    ...
    return {
      ...
      growth,
      lastAttempt: courseAttempts[0]?.createdAt?.toISOString() ?? null
    };
  });
