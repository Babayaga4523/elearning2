...
export async function uploadPdfAction(formData: FormData) {
  try {
    ...
    // Validate PDF magic number (Security: Prevent malicious file upload)
    ...
    if (!isPDF) {
      return { success: false, error: "File bukan PDF yang valid (magic number check failed)" };
    }

    // Additional validation: Check for PDF EOF marker
    const bufferStr = buffer.toString('latin1');
    if (!bufferStr.includes('%%EOF')) {
      return { success: false, error: "File PDF tidak valid atau corrupt" };
    }
...
export async function upsertModule(
  ...
  try {
    ...
    if (moduleId) {
      ...
      if (existing && existing.type === "PDF" && (validatedData.type === "VIDEO" || (validatedData.type === "PDF" && existing.url !== validatedData.tempPath))) {
...
export async function deleteModule(courseId: string, moduleId: string) {
  try {
    ...
    if (moduleData && moduleData.type === "PDF" && moduleData.url) {
      await deleteFileFromDisk(moduleData.url);
    }
...
export async function reorderModules(courseId: string, orderedIds: string[]) {
  try {
    ...
    await db.$transaction(async (tx) => {
      // Lock the course row to prevent concurrent reorders
      await tx.course.update({
        where: { id: courseId },
        data: { updatedAt: new Date() }
      });

      // Verify all modules belong to this course (security check)
      ...
      if (modules.length !== orderedIds.length) {
        throw new Error("Invalid module IDs detected");
      }
    ...
    return { success: false, error: error?.message ?? "Gagal menyusun ulang modul" };
  }
}
