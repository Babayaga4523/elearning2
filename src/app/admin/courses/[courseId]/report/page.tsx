...
export default async function CourseReportPage({ params }: Props) {
  ...
  const scoreDistribution = Array.from({ length: 10 }, (_, i) => {
    ...
    return {
      ...
      pre: (testAttempts as any[]).filter((a: any) => a.test.type === "PRE" && a.score >= lo && a.score <= hi).length,
      post: (testAttempts as any[]).filter((a: any) => a.test.type === "POST" && a.score >= lo && a.score <= hi).length,
    };
  });
